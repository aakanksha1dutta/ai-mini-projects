...
def extract(inputLines):
    ...
    for line in inputLines:
        ...
        for c in line.split(','):
            #only add positive
            literals.add(c if c[0]!='~' else c[1:]) 
            currClause.append(c)
...
def pure_heuristic(clauses, assignment):
    pure = {}
    deleted = []
...
def unit_heuristic(clauses, assignment):
    ...
    for i in range(len(clauses2)):
        ...
        if len(clause)==1:
            c = clause[0] 
            if c[0]=="~":
                #add to assignment if it does not exist
                #if it exists, then it must be from contradicting unit clauses and it is unsat
...
def DPLL(clauses, literals, model,length,doUnit=True, doPure=True):
    ...
    if len(literals)==0: #all assigned
        ...
        if entailTrue:
            #assign the unassigned literals to True
    ...
    if doPure and model=={}:
        pure = pure_heuristic(clauses, model)
        model.update(pure)
